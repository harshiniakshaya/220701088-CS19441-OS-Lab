# 220701088-CS19441-OS-Lab
### Exp 2:
- [System monitoring using shell script](Ex-02)
### Exp 3:
- [Employee average pay](Ex-03/a)
- [Results of an examination](Ex-03/b)
### Exp 4:
- [Signal catching](Ex-04)
### Exp 5:
- [System call tracing](Ex-05/systemcall.sh)
### Exp 6:
- [IPC using shared memory - sender.c](Ex-06/sender.c)
- [IPC using shared memory - receiver.c](Ex-06/receiver.c)
### Exp 7 - Scheduling Techniques:
- [First In First Out](Ex-07/fcfs.c)
- [Shortest Job First](Ex-07/sjf.c)
- [Priority Scheduling](Ex-07/ps.c)
- [Round Robin](Ex-07/rr.c)
### Exp 8:
- [Produce-Consumer problem using semaphores](Ex-08/semaphore.c)
### Exp 9:
- [Deadlock Avoidance - Banker's Algorithm](Ex-09/deadlock.c)
### Exp 10 - Memory Allocation Techniques:
- [Best Fit](Ex-10/best_fit.c)
- [First Fit](Ex-10/first_fit.c)
### Exp 11 - Page Replacement Techniques:
- [First In First Out](Ex-11/fifo.c)
- [Least Recently Used](Ex-11/lru.c)
